    ...
    try {
        ...
        if (req.method === 'POST') {
            ...
            const existing = await pool.request()
                ...
                .query('SELECT TOP 1 * FROM InsuranceCompanies WHERE Name = @Name');
            ...
            const sortOrderResult = await pool.request()
                .query('SELECT ISNULL(MAX(SortOrder), 0) + 1 AS NextSortOrder FROM InsuranceCompanies');

            const result = await pool.request()
                .input('Name', sql.NVarChar(255), name)
                .input('SortOrder', sql.Int, sortOrderResult.recordset[0].NextSortOrder)
                .query(`
                    INSERT INTO InsuranceCompanies (Name, SortOrder)
                    OUTPUT INSERTED.*
        ...
        if (req.method === 'DELETE' && id) {
            await pool.request()
                .input('Id', sql.Int, id)
                .query('DELETE FROM InsuranceCompanies WHERE Id = @Id');
        ...
        context.log.error('insurance-companies error:', err.message);
        context.res = {
            status: 500,
            headers: { ...CORS, 'Content-Type': 'application/json' },
